...
export default function LoadingScene(props: Props) {
  ...
  const startup = async () => {
    ...
    if (!store.debate.topic || store.debate.contribution === -1)
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      const hasMatch = store.debate.match && store.debate.match!.sync;
  ...
  useEffect(() => {
    console.log(
      ...
      'hasstream:',
      !!stream,
      'startedHandshake:' + state.startedHandshake
    );
    if (store.debate.match && stream && !state.startedHandshake) {
      if (state.unloadFlag.flag) return;
